...
from pystream.constants import PACKET_MAX_SIZE, PACKET_HEADER_SIZE
from pystream.exceptions import InvalidIPAddressError, InvalidPortError, InvalidPacketSizeError
...
def validate_ipv4_address(value: str) -> None:
    ...
    try:
        ...
        raise InvalidIPAddressError(f"Type of value must be string, but got {type(value)}") from e
    except OSError as e:
        raise InvalidIPAddressError(f"Format of value must be IPv4, but got {value}") from e
...
def validate_port(value: int) -> None:
    ...
    if not __is_exact_int(value):
        ...
        raise InvalidPortError(f"Port must be between 0 and 65535 inclusive, but got {value}")
...
def validate_packet_size(value: int) -> None:
    ...
    """
    if not __is_exact_int(value):
        raise InvalidPacketSizeError(f"Packet size must be an integer, but got {type(value)}")
